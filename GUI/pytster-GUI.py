try:
  ...
  #website1:atdata
    ...
    querystring = {"email":i,"api_key":"<YOUR API KEY>"}
  ...
  #Website2:hunterio
    url = f"https://api.hunter.io/v2/email-verifier?email={i}&api_key=<YOUR API KEY>"
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    ...
    hint("IP_search.txt")
  #Website1:IPstack
    url=f"http://api.ipstack.com/{i}?access_key=<YOUR API KEY>"
    response=requests.get(url)
    write("IP_search.txt",response.json(),'ab','IPstack')
    end("IP_search.txt")

  #Website2:Ip2location
    url="https://api.ip2location.io/?key=<YOUR API KEY>8&ip"+i
    response=requests.get(url)
    #/
  ...
  #Website3:ipapi
    url='http://api.ipapi.com/api/8.8.8.8/?access_key=<YOUR API KEY>'
    response=requests.get(url)
  ...
  #Website1:IP2WHOIS
    url="https://api.ip2whois.com/v2?key=<YOUR API KEY>&domain="+i
    response=requests.get(url)
    write("Domain_search.txt",json.loads(response.text),"ab","IP2WHOIS:")
    end("Domain_search.txt")

  #Website2:WhoisFreaks
    url="https://api.whoisfreaks.com/v1.0/whois?apiKey=<YOUR API KEY>&whois=live&domainName="+i
    response=requests.get(url)
